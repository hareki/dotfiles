...
local opts =
  { iterations = 7, warmup = 2, layout = 'both', fragment = false, profile = false, json = nil }
local files = {}
for i = 1, #(arg or {}) do
  local a = arg[i]
...
local function render(input, layout)
  return core.render(
    input,
    { cwd = cwd, cols = 120, layout = layout, force_fragment = opts.fragment }
  )
end
...
io.write(
  string.format(
    'corpus: %d file(s)  iterations: %d  warmup: %d  mode: %s\n\n',
    #files,
    opts.iterations,
    opts.warmup,
    opts.fragment and 'fragment' or 'full'
  )
)
io.write(
  string.format('%-24s %-13s %9s %9s %9s %9s\n', 'file', 'layout', 'median', 'min', 'max', 'KiB/s')
)
