...
---@return WinConfig
function M.get_lg_popup_size(with_border)
    local screen_w = vim.opt.columns:get()
    local screen_h = vim.opt.lines:get() - vim.opt.cmdheight:get()
    local window_w = math.floor(screen_w * Constant.ui.popup_size.lg.WIDTH)
    local window_h = math.floor(screen_h * Constant.ui.popup_size.lg.HEIGHT)
