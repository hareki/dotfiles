...
function M.buffer_entry_maker(opts)
  ...
  -- TODO: refactor this chatgpted code
  return function(item)
    ...
    local combined = display_directory .. file_name
    ...
    entry.display = function()
      ...
      -- Precisely locate the combined path segment in the rendered line to place dir/file highlights
      local combined_start = line:find(combined, 1, true) - 1
      if combined_start then
        local dir_len = #display_directory
        local file_len = #file_name

        if dir_len > 0 then
          -- Highlight only the directory portion (including trailing slash)
          table.insert(hl, { { combined_start, combined_start + dir_len }, 'SnacksPickerDir' })
        end

        -- Highlight only the filename portion
        local file_start = combined_start + dir_len
